...
impl ARM9 {
    ...
    fn data_proc<const I: bool, const S: bool>(&mut self, hw: &mut HW, instr: u32) {
        ...
        } else if special_change_status { self.regs.restore_cpsr() }
        ...
        if opcode & 0xC != 0x8 {
            ...
            } else { self.regs[dest_reg] = result }
        }
        if !clocked {
            if temp_inc_pc { self.regs[15] = self.regs[15].wrapping_sub(4) } // Dec after temp inc
            self.instruction_prefetch::<u32>(hw, AccessType::S);
        }
    }
    ...
    fn psr_transfer<const I: bool, const P: bool, const L: bool>(&mut self, hw: &mut HW, instr: u32) {
        ...
        if msr {
            ...
            if use_spsr { set(self.regs.spsr_mut()) } else {
            ...
            self.regs[instr >> 12 & 0xF] = if use_spsr { self.regs.spsr() } else { self.regs.cpsr() };
    ...
    fn mul_long<const U: bool, const A: bool, const S: bool>(&mut self, hw: &mut HW, instr: u32) {
        ...
        if accumulate {
            self.internal();
            (self.regs[src_dest_reg_high] as u64) << 32 |
            self.regs[src_dest_reg_low] as u64
        } else { 0 });
        if change_status {
    ...
    fn single_data_transfer<const I: bool, const P: bool, const U: bool,
        ...
        if pre_offset {
            exec(offset_applied);
            if write_back { self.regs[base_reg] = offset_applied }
        } else {
    ...
    fn block_data_transfer<const P: bool, const U: bool, const S: bool, const W: bool, const L: bool>
        ...
        if num_regs == 0 {
            ...
            if write_back { self.regs[base_reg] = final_addr }
        }
